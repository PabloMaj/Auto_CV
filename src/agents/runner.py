...
class RunnerAgent:
    ...
    def run(self, state):
        ...
        if not generated_code.strip():
            ...
            state["execution_error"] = ("Generated code is empty.")
            return state
        ...
        try:
            ...
            logger.info(f"Execution finished (success={execution_success})")

        except subprocess.TimeoutExpired:
            state["execution_success"] = False
            state["execution_error"] = (f"Execution timeout after {self.timeout} seconds")
