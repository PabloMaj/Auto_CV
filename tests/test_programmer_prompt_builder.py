...
from src.agents.funcs.programmer_prompt_builder import ProgrammerPromptBuilder, ProgrammerReasoningType
...
def mock_prompts(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.agents.funcs.programmer_prompt_builder.BASE_PROGRAMMER_PROMPT",
        "BASE:{user_prompt}|{desired_output_specification}|{code_start_token}|{code_end_token}",
    )

    monkeypatch.setattr(
        "src.agents.funcs.programmer_prompt_builder.INITIAL_CODING_PROMPT",
        "INIT:{raw_images_placeholder}|{base_prompt}",
    )

    monkeypatch.setattr(
        "src.agents.funcs.programmer_prompt_builder.BUG_FIXING_PROMPT",
        "BUG:{previous_code}|{runner_error}|{base_prompt}",
    )

    monkeypatch.setattr(
        "src.agents.funcs.programmer_prompt_builder.IMPROVING_BASED_ON_SUGGESTION_PROMPT",
        "IMPROVE:{previous_code}|{verified_problems}|{improvement_suggestions}|{base_prompt}",
    )

    monkeypatch.setattr(
        "src.agents.funcs.programmer_prompt_builder.NOVELTY_CODING_PROMPT",
